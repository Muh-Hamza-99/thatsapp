...
const SidebarChatList = ({ friends, sessionId }: SidebarChatListProps) => {
  ...
  useEffect(() => {
    ...
    const chatHandler = (message: ExtendedMessage) => {
        ...
        setUnseenMessages(previous => [...previous, message]);
    };
  ...
  useEffect(() => {
    if (pathname?.includes("chat")) setUnseenMessages(previous => previous.filter(message => !pathname.includes(message.senderId)));
  }, [pathname]);
                ...
                return (
                    <li key={friend.id}>
                        <a href={`/dashboard/chat/${chatHrefConstructor(sessionId, friend.id)}`} className="text-gray-700 hover:text-indigo-600 hover:bg-gray-50 group flex items-center gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold">
                            {friend.name}
                            {unseenMessagesCount > 0 ? (<div className="bg-indigo-600 font-medium text-xs text-white 2-4 h-4 rounded-full flex justify-center items-center">{unseenMessagesCount}</div>) : null}
                        </a>
                    </li>
                )
            })
