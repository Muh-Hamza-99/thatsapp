...
const Messages = ({ initialMessages, sessionId, sessionImage, chatPartner, chatId }: Props) => {
  ...
  const formatTimestamp = (timestamp: number): string => format(timestamp, "hh:mm a");
                    ...
                    <div className="chat-message" key={`${message.id}-${message.timestamp}`}>
                        <div className={cn("flex items-end", { "justify-end": isCurrentUser })}>
                            ...
                            <div className={cn("relative w-6 h-6", {
                                ...
                                <Image src={isCurrentUser ? (sessionImage as string) : chatPartner.image} fill alt="Profile picture" referrerPolicy="no-referrer" className="rounded-full" />
                            </div>
                        </div>
                    </div>
                )
